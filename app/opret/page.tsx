          ...
          {success ? (
            <div className="text-center animate-in fade-in zoom-in duration-300">
              ...
              <div className="bg-blue-50 p-4 rounded-xl text-sm text-blue-800 mb-8 border border-blue-100 flex items-start text-left gap-3">
                ...
                <span>Du skal klikke på linket i mailen for at aktivere din konto, før du kan logge ind.</span>
              </div>
              ...
              <form onSubmit={handleSignup} className="w-full flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1 px-4 mt-2">
                  {!isEmailValid && email.length > 0 && (
                    <p className="text-red-500 text-xs flex items-center gap-1"><i className="fa-solid fa-circle-exclamation"></i> Ugyldig email.</p>
                  )}
                  {!isPasswordStrong && password.length > 0 && (
                    <p className="text-red-500 text-xs flex items-center gap-1"><i className="fa-solid fa-circle-exclamation"></i> Password er for kort.</p>
                  )}
                  {!passwordsMatch && confirm.length > 0 && (
                    <p className="text-red-500 text-xs flex items-center gap-1"><i className="fa-solid fa-circle-exclamation"></i> Passwords matcher ikke.</p>
                  )}
                </div>
                ...
                >
                  {loading ? (
                    <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> Opretter...</>
                  ) : (
                    'Opret bruger'
                  )}
                </button>
