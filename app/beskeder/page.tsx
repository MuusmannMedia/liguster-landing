...
function BeskederContent() {
  ...
  // DM "soft delete" state (thread_id -> deleted_at)
  ...
  const upsertThreadToTop = async (opts: {
    ...
    // Hvis tråden er "soft-deleted" og beskeden er ældre end deleted_at (burde ikke ske), ignorér
    ...
    // Hvis den ikke findes, hent user-info og tilføj
  ...
  const handleSend = async () => {
    ...
    // DM: sørg for tråd ligger øverst (og bliver synlig igen hvis den var "slettet for mig")
    if (isDirectMessage && dmTargetUser) {
  ...
  // ✅ NY: "Slet chat" = soft-delete for DM (kun for mig)
  const handleDeleteThread = async () => {
    ...
    if (isDirectMessage) {
      ...
      const { error } = await supabase
        ...
        .upsert(
          { thread_id: activeThreadId, user_id: userId, deleted_at: deletedAt },
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // ✅ Hent DM soft-delete state til map
      ...
      // ✅ DM threads ud fra messages, men filtrér beskeder før deleted_at væk
      ...
      if (dms && dms.length > 0) {
        ...
        for (const m of dms) {
          ...
          if (delAt && new Date(m.created_at).getTime() <= new Date(delAt).getTime()) {
            // Brugeren har "slettet" tråden efter denne besked -> ignorér denne (og evt hele tråden)
            continue;
          }
      ...
      if (dmUserIdFromUrl) {
        ...
        if (tar) {
          ...
          const { data: ex } = await supabase
            ...
            .or(
              `and(sender_id.eq.${cId},receiver_id.eq.${dmUserIdFromUrl}),and(sender_id.eq.${dmUserIdFromUrl},receiver_id.eq.${cId})`
            )
            .order('created_at', { ascending: false })
            .limit(1);

          if (ex && ex.length > 0) {
            handleSelectThread(ex[0].thread_id, true, cId, dmUserIdFromUrl);
            ...
            if (adm && adm.length > 0) {
              handleSelectThread(adm[0].thread_id, true, cId, dmUserIdFromUrl);
            }
          }
        }
      }
    ...
    const fetchM = async () => {
      ...
      if (isDirectMessage) {
        ...
        setMessages(
          data.map((m: any) => ({
            ...m,
            users: uMap[m.user_id] || { name: '?', avatar_url: null },
          })) as any
        );
        scrollToBottom();
        return;
  ...
  // --- REALTIME: aktiv tråd (viser nye beskeder i åbent vindue) ---
  useEffect(() => {
    if (!activeThreadId || !userId) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const senderId = newMsg[senderField];

          // ignorer vores egen hvis vi allerede lagde optimistisk ind
          if (senderId === userId) return;

          // DM: respekter soft-delete (hvis beskeden ikke er nyere end deleted_at, ignorér)
          if (isDirectMessage) {
            const deletedAt = getDmDeletedAt(activeThreadId);
            if (deletedAt && new Date(newMsg.created_at).getTime() <= new Date(deletedAt).getTime()) return;
          }
  ...
  // ✅ NY: REALTIME DM inbox (så tråde kan dukke op igen uden reload)
  useEffect(() => {
    if (!userId) return;
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
              {threads.map((t) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl flex flex-col gap-1 ${
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {!!t.unreadCount && t.unreadCount > 0 && (
                      <span className="min-w-6 h-6 px-2 rounded-full bg-black text-white text-[11px] font-black flex items-center justify-center">
                        {t.unreadCount}
                      </span>
                    )}
                  </div>
