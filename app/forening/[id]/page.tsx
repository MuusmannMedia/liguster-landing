...
type Forening = {
  ...
  images?: any; // kan være string/array afhængigt af DB
  oprettet_af?: string;
  slug?: string;
  is_public?: boolean;
};
...
// ✅ NY: Forening-hero billeder kan være enten fuld URL eller storage path.
// Vi sørger for at konvertere storage paths til public URL.
const getForeningImageUrl = (path: string | null | undefined) => {
  if (!path) return '';
  if (path.startsWith('http')) return path;
...
// ✅ NY: Normaliser "images" fra DB (kan være array, JSON-string, CSV-string, null)
const normalizeImagesFromDb = (imagesField: any, fallbackCover?: string | null) => {
  let arr: string[] = [];

  if (Array.isArray(imagesField)) {
    arr = imagesField.filter(Boolean).map(String);
  } else if (typeof imagesField === 'string' && imagesField.trim()) {
    const s = imagesField.trim();

    // Forsøg JSON parse først
    try {
      const parsed = JSON.parse(s);
      if (Array.isArray(parsed)) arr = parsed.filter(Boolean).map(String);
      else if (typeof parsed === 'string') arr = [parsed];
    } catch {
      // fallback: split på komma (hvis nogen har gemt som csv)
      if (s.includes(',')) arr = s.split(',').map((x) => x.trim()).filter(Boolean);
      else arr = [s];
    }
  }

  // Fallback til billede_url hvis images er tomt
  if (arr.length === 0 && fallbackCover) arr = [fallbackCover];
...
export default function ForeningDetaljePage() {
  ...
  useEffect(() => {
    async function loadAllData() {
      ...
      try {
        ...
        // ✅ FIX (2): robust load af billeder (images kan være array/string)
        const rawImages = normalizeImagesFromDb(foreningData.images, foreningData.billede_url ?? null);

        // ✅ Konverter til visnings-URLs (hvis noget er storage path)
        const resolved = rawImages.map((x) => getForeningImageUrl(x));

        setHeroImages(resolved);

        // Sørg for at active index er valid efter refresh
        setActiveHeroIndex((prev) => {
          const next = Math.min(prev, Math.max(0, resolved.length - 1));
  ...
  const onLightboxTouchEnd = (e: TouchEvent<HTMLDivElement>) => {
    ...
    // ignorér hvis det mest er vertical scroll
    if (Math.abs(dy) > Math.abs(dx)) return;

    const threshold = 45; // swipe-følsomhed
    if (dx <= -threshold) nextHeroImage();
    if (dx >= threshold) prevHeroImage();
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // ✅ Bedre filnavn: bevar extension hvis muligt
    const ext = file.name.includes('.') ? file.name.split('.').pop() : '';
    const fileName = `${realForeningId}_${Date.now()}${ext ? `.${ext}` : ''}`;

    const { error: uploadError } = await supabase.storage.from('foreningsbilleder').upload(fileName, file);

    if (!uploadError) {
      // Vi gemmer gerne "path" i DB (robust), men viser publicUrl i UI.
      const publicUrl = getForeningImageUrl(fileName);

      // Opdater lokalt (UI bruger URLs)
      const newImages = [...heroImages, publicUrl];
      setHeroImages(newImages);
      setActiveHeroIndex(newImages.length - 1);

      // ✅ Gem i DB (robust ift. images kolonne + fallback)
      // Vi gemmer i images som "path" hvis vi kan — men for kompatibilitet
      // forsøger vi at gemme det i samme format som allerede ligger i DB.
      const existingRaw = normalizeImagesFromDb((forening as any)?.images, forening?.billede_url ?? null);

      // Hvis eksisterende ligner URLs, så gem URLs; ellers gem paths.
      const existingLooksLikeUrls = existingRaw.some((x) => String(x).startsWith('http'));
      const nextToStore = existingLooksLikeUrls ? newImages : [...existingRaw.map(String), fileName];

      let updateRes = await supabase
        .from('foreninger')
        .update({
          billede_url: existingLooksLikeUrls ? newImages[newImages.length - 1] : fileName,
          images: nextToStore,
        })
        .eq('id', realForeningId);

      // Fallback hvis images-kolonne ikke findes / ikke kan opdateres
      if (updateRes.error) {
        ...
        if (fallback.error) {
          alert('Billedet blev uploadet, men kunne ikke gemmes i databasen (tjek RLS/kolonner).');
        }
      }

      // Opdater forening state lidt (så edit mode viser korrekt)
      setForening((prev) =>
        prev
          ? {
              ...prev,
  ...
  const handleDeleteHeroImage = async (indexToDelete: number) => {
    ...
    // Best effort: opdater DB konsistent med eksisterende format
    const existingRaw = normalizeImagesFromDb((forening as any)?.images, forening?.billede_url ?? null);
    const existingLooksLikeUrls = existingRaw.some((x) => String(x).startsWith('http'));

    // Hvis DB gemmer URLs, kan vi direkte gemme newImages (URLs).
    // Hvis DB gemmer paths, så kan vi desværre ikke udlede paths fra public URLs her.
    // Derfor: hvis det ikke ligner URLs, gemmer vi kun cover fallback i billede_url (path ukendt).
    if (existingLooksLikeUrls) {
      await supabase
        .from('foreninger')
        .update({
          images: newImages,
      ...
      // Fallback: kun cover i DB (paths kan ikke rekonstrueres fra public URLs)
      await supabase
        .from('foreninger')
        .update({
          billede_url: null,
  ...
  const handleJoin = async () => {
    ...
    await supabase.from('foreningsmedlemmer').insert([
      { forening_id: realForeningId, user_id: userId, rolle: 'medlem', status: 'pending' },
    ]);
    ...
    <div className="min-h-screen flex flex-col bg-[#869FB9]">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto p-4 pb-20 space-y-6">
        ...
        <div className="bg-white rounded-[24px] p-5 shadow-md mt-6 flex flex-col gap-4">
          {/* ✅ (1) Desktop hero 25% højere:
              før: md:aspect-[16/10] (h=0.625)
              nu:  md:aspect-[32/25] (h=0.78125 = +25%)
          */}
          <div className="relative w-full aspect-square md:aspect-[32/25] rounded-[18px] overflow-hidden bg-gray-100 group">
            {heroImages.length > 0 ? (
              <>
                <img
                ...
                {heroImages.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm"
                    ...
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm"
                      ...
                      {heroImages.map((_, idx) => (
                        <div
                          ...
                          className={`w-1.5 h-1.5 rounded-full transition-all ${
                            idx === activeHeroIndex ? 'bg-white w-3' : 'bg-white/50'
                          }`}
                        />
                      ))}
                    </div>
      ...
      {/* --- LIGHTBOX (Fuldskærm + swipe) --- */}
      {lightboxOpen && heroImages.length > 0 && (
        <div
          className="fixed inset-0 z-[400] bg-black flex items-center justify-center animate-in fade-in duration-200"
        ...
        >
          ...
          {heroImages.length > 1 && (
            <>
              <button
                className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 text-white/90 hover:text-white text-3xl md:text-4xl p-4 z-20"
                onClick={(e) => {
                  e.stopPropagation();
                  prevHeroImage();
                }}
              ...
              <button
                className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 text-white/90 hover:text-white text-3xl md:text-4xl p-4 z-20"
                onClick={(e) => {
                  e.stopPropagation();
                  nextHeroImage();
                }}
              ...
              <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-2 z-20">
                {heroImages.map((_, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${idx === activeHeroIndex ? 'w-6 bg-white' : 'w-2 bg-white/50'}`}
          ...
          <div className="absolute bottom-2 left-0 right-0 text-center text-white/60 text-[11px] font-bold z-10">
            Swipe ↔︎ • Piletaster • Esc
          </div>
        </div>
      )}
